...
interface TripsContext {
  getTrips: (callback: (response: Response) => void) => Promise<void>;
  createTrip: (
    trip: Trip,
...
export function TripsProvider({ children }: { children: React.ReactNode }) {
  ...
  async function getTrips(callback: (response: Response) => void) {
  ...
  async function joinTrip(
    ...
    const options = createFetchRequestOptions({}, 'PUT');
    const response = await fetch(`${API_URL}/trip`, options);
